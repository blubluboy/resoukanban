...
HOTLIST_SOURCE = 'weibo'、'zhihu'   # 你可以改为 'weibo'、'zhihu' 等

# 3. 天气城市设置
# 高德天气城市代码（广州：440100，北京：110000）
CITY_ADCODE = "440100"   # 修改为广州代码

# 日出日落位置（支持拼音，如 "Guangzhou"）
WTTR_LOCATION = "Guangzhou"

# 4. 屏幕显示文字
# 天气页面左上角的自定义标题
CITY_DISPLAY_NAME = "广州 | 羊城"
...
def get_hotlist_data(source):
    ...
    try:
        if source == "zhihu":
            ...
            # 修复：使用B站热门视频接口（稳定返回标题）
            url = "https://api.bilibili.com/x/web-interface/popular?ps=20"
            res = requests.get(url, headers=HEADERS, timeout=10).json()
            if res.get('code') == 0:
                titles = [item['title'] for item in res['data']['list']]
            else:
                titles = ["B站数据解析失败"]
        elif source == "github":
            ...
            res = requests.get(url, headers=HEADERS, timeout=10).json()
            titles = [f"{item['full_name']}: {item['description'][:50] if item['description'] else 'No desc'}" for item
                      in res['items']]
        elif source == "weibo":
            # 新增：微博热搜接口（无需认证）
            url = "https://weibo.com/ajax/side/hotSearch"
            res = requests.get(url, headers=HEADERS, timeout=10).json()
            if 'data' in res and 'realtime' in res['data']:
                # 提取热搜词，过滤掉广告（如果有flag_ad）
                realtime = res['data']['realtime']
                titles = [item['word'] for item in realtime if not item.get('flag_ad', False)]
            else:
                titles = ["微博数据解析失败"]
        else:
            titles = ["不支持的数据源"]
...
# --- 任务：热搜看板 ---
def task_hotlist():
    source_map = {"zhihu": "知乎热榜", "bilibili": "B站热门", "github": "GitHub 热门", "weibo": "微博热搜"}
